...
This is a MERN full-stack web-application.  The front-end is displayed using React.js with client-side routing provided by the react-router-dom npm package.  Server-side routing is performed with the express npm package.  The development environment was provided by Node.js.  And, persistent data is stored in a MongoDB database.  Users are taken to the homepage where they can choose to search the Google Books API or see saved books.  The saved page provides a search compnent that makes a call to the Google Books API and returns book information that is dynamically displayed.  The search page connects to the MongoDB database to pull information on books that have been saved previously and synamically display them.  There is a "remove" button in case duplicate books have been saved, or the user wants to delete a book from the MongoDB database.
...
If you want to use this code for yourself please clone/fork this repository.

## Usage 

The [Google Books API](https://github.com/etorres-revature/React-Google_Books) is [deployed on Heroku](https://lit-citadel-69293.herokuapp.com/).

## Guidelines for Contributing

If you have thoughts on futher features or any suggestions on how to imporve the [Google Books API Search](https://github.com/etorres-revature/React-Google_Books) please send them to the e-mail address listed below.
...
This *Google Books API Search* was built by :green_heart: Eric D. Torres :green_heart:
